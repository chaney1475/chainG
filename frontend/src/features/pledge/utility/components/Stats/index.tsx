...
import { ContentContainer, DisabledColorText, LowColorText, StatusContainer, StatusIcon, TextBox, TextContainer, TopDescription } from './styles'
...
export function Stats() {
  ...
  const date = new Date();
  const status = utilityInfo?.currentWeek.find((item) => item.userId === userId)?.status
  const dayOfWeek = date.getDay();

  function getStatus(dayOfWeek: number, status: boolean | undefined) : BudgetStatus {
  ...
  return (
    <>
      <BoxContainer>
        <ContentContainer>
          <TopDescription>공과금 - {month}월 {weekOfMonth}주차 정산내역</TopDescription>
          <TextContainer>
            <TextBox>
            ...
            <TextBox>
              ...
              <DisabledColorText>{t(`duty.week.${utilityInfo?.dueDayOfWeek}`)}요일</DisabledColorText>
            </TextBox>
            <TextBox>
              <LowColorText>공과금 납부 여부</LowColorText>
              <StatusContainer>
                <StatusIcon variant={getStatus(dayOfWeek, status)} />
                <LowColorText>{t(`pledge.status.${getStatus(dayOfWeek, status)}`)}</LowColorText> 
              </StatusContainer>
            </TextBox>
